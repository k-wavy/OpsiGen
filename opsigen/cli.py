...
from .config import FunctionalTrainingConfig, PredictionConfig, PreprocessRunConfig, TrainingConfig
from .dataset_prep import prepare_wds_dataset
from .exceptions import OpsiGenError
from .functional_training import train_functional_classifier_from_config
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_functional_parser = subparsers.add_parser(
        "train-functional",
        help="Train a binary functional/non-functional classifier.",
    )
    train_functional_parser.add_argument(
        "--config",
        required=True,
        type=Path,
        help="Functional classifier training config JSON/YAML path.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        if args.command == "train-functional":
            result = train_functional_classifier_from_config(args.config, verbose=args.verbose)
            print(f"Functional classifier training complete: {result.output_dir}")
            print(f"Final checkpoint: {result.final_checkpoint_path}")
            if result.best_checkpoint_path:
                print(f"Best checkpoint: {result.best_checkpoint_path}")
            return
        ...
        if args.command == "prepare-wds":
            ...
            print(f"Functional classifier config: {result.functional_train_config}")
